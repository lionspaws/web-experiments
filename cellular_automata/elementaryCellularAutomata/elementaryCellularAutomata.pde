...
void draw() {
  // only do it once or it swims away!
  if(!drawnOnce){
    background(0);
    ...
    for (int i = 0; i < cols; i++) {
      for (int j = 0; j < rows; j++) {
        ...
        // or the topmost row
        if(j == 0) {
          cellUpLeft = grid[xCompL][j].filled;
          cellUpCentre = grid[xCompC][j].filled;
          cellUpRight = grid[xCompR][j].filled;
        }
        else {
          cellUpLeft = grid[xCompL][j-1].filled;
          cellUpCentre = grid[xCompC][j-1].filled;
